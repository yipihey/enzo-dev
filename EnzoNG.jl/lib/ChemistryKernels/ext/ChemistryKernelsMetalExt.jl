...
`:metal` backend and specialises the device-array helpers onto `MtlArray`. Metal
is Float32-only, so callers must request `Float32` element types — stiff
chemistry in f32 is the same accuracy compromise GPU-Grackle makes, and the
parity layer flags where it bites.
"""
